...
from getters.s3_getters import load_s3_data, get_shapefile_from_s3
from asf_heat_pump_suitability.getters import get_datasets

# Constants
CRS = "EPSG:4326"  # WGS84 - standard longitude/latitude projection
TAKEUP_RATE = 30  # Percentage of households in each LSOA to install heat pumps
POWER_PER_HEATPUMP = 3.5  # Power consumption per heat pump in kW
...
def _process_shapefile(shapefile_directory: str) -> gpd.GeoDataFrame:
    """
...
def assess_heatpump_suitability(
    ...
    power_per_heatpump: float,
    takeup_rate: float,
    voltage_factor: float = 0.95,
) -> pd.DataFrame:
    """
    Assess the suitability of LSOAs for heat pump installations based on grid capacity.
    ...
    Args:
        ...
        takeup_rate: Percentage of households to install heat pumps (0-100).
        voltage_factor: Factor to account for voltage drop (default 0.95).

    Returns:
        DataFrame with suitability assessment for each LSOA.
    """
    # Convert headroom from MVA to kW
    lsoa_data["headroom_kw"] = lsoa_data["distributed_headroom"] * 1000 * voltage_factor

    # Calculate number of households to install heat pumps
    lsoa_data["heatpump_installations"] = np.round(
        lsoa_data["household_count"] * (takeup_rate / 100)
    )

    # Calculate total power required for heat pump installations
    lsoa_data["power_required_kw"] = (
        lsoa_data["heatpump_installations"] * power_per_heatpump
    )

    # Determine if LSOA is suitable for heat pump installations
    lsoa_data["has_grid_capacity"] = (
        lsoa_data["headroom_kw"] >= lsoa_data["power_required_kw"]
    )

    # Calculate excess or deficit capacity
    lsoa_data["capacity_difference_kw"] = (
        lsoa_data["headroom_kw"] - lsoa_data["power_required_kw"]
    )

    # Calculate percentage of required capacity available
    lsoa_data["capacity_percentage"] = (
        lsoa_data["headroom_kw"] / lsoa_data["power_required_kw"] * 100
    ).clip(upper=100)
...
def generate_npg_df() -> gpd.GeoDataFrame:
    ...
    return npg_df[
        ["id", "firm_capacity_load_mva", "maximum_demand_mva", "geo_shape"]
    ].rename(
        columns={
...
def generate_spen_df() -> gpd.GeoDataFrame:
    ...
    return spen_df[
        ["substation_name", "firm_capacity_mva", "maximum_load_mva", "geo_shape"]
...
def generate_ssen_df() -> gpd.GeoDataFrame:
    ...
    sepd_shape = _process_shapefile(
        "source_data/grid_operators/SSEN/SEPD Primary Sub Boundaries"
    )
    shepd_shape = _process_shapefile(
...
def generate_ukpn_df() -> gpd.GeoDataFrame:
    ...
    return ukpn[
        ["primary", "firm_capacity_mva", "maximum_load_mva", "geo_shape"]
...
def calculate_grid_capacity() -> pd.DataFrame:
    ...
    households_df = households_df.rename(
        columns={"mnemonic": "LSOA21CD", "2021": "household_count"}
    ).to_pandas()
    ...
    result = assess_heatpump_suitability(headroom_df, POWER_PER_HEATPUMP, TAKEUP_RATE)
