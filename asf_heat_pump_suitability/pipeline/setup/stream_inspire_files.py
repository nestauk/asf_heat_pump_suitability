"""Stream INSPIRE land registry files to S3.

Streams INSPIRE land registry files for Scotland (from ROS webpage) and/or
England and Wales (from government website) to the asf-heat-pump-suitability S3
bucket. Files are unzipped during streaming.

Run:
    uv run ahps-stream-inspire -n all

Nation options (-n / --nations):
    ew      England and Wales only
    s       Scotland only
    all     Both England & Wales and Scotland
"""

import argparse
import time
import zipfile

import boto3
import regex as re
import requests
from bs4 import BeautifulSoup
from tqdm import tqdm

from asf_heat_pump_suitability import config
from asf_heat_pump_suitability.getters import base_getters


def parse_arguments() -> argparse.Namespace:
    """Create ArgumentParser and parse.

    Returns:
        argparse.Namespace: populated Namespace.
    """
    parser = argparse.ArgumentParser(description="Stream INSPIRE land registry files to S3.")
    parser.add_argument(
        "-n",
        "--nations",
        help="Nations to download INSPIRE files for.",
        type=str,
        choices=["ew", "s", "all"],
        required=True,
    )
    return parser.parse_args()


def main() -> None:
    """Entry point registered as the ``ahps-stream-inspire`` console script."""
    args = parse_arguments()

    s3 = boto3.client("s3")
    bucket = "asf-heat-pump-suitability"

    if args.nations in ["s", "all"]:
        ros_url = config["data_source"]["S_ros_inspire_url"]
        page = requests.get(ros_url)
        soup = BeautifulSoup(page.content, "html.parser")

        pattern = re.compile(r'https:.+?(?=")')
        url_prefix = pattern.search(soup.find("script", string=pattern).text).group(0)
        ids = {v.contents[0]: v["value"] for v in soup.find_all("option") if v["value"]}
        download_urls = {area: url_prefix + url for area, url in ids.items()}

        for area, url in tqdm(download_urls.items()):
            content = base_getters.get_content_from_url(url)
            z = zipfile.ZipFile(content)
            for file in z.namelist():
                with z.open(file) as f:
                    s3.upload_fileobj(
                        Fileobj=f,
                        Bucket=bucket,
                        Key=f"source_data/inspire_scotland/{area}/{file}",
                    )

    if args.nations in ["ew", "all"]:
        page = requests.get(
            url=config["data_source"]["EW_inspire_url"],
            headers={"User-Agent": "Data collection for research."},
        )
        soup = BeautifulSoup(markup=page.content, features="html.parser")

        urls = [a.get("href") for a in soup.find_all(name="a", attrs={"aria-current": False})]
        urls = [url for url in urls if url and url.endswith(".zip")]
        url_prefix = "https://use-land-property-data.service.gov.uk/"

        for url in tqdm(urls):
            area = url.split("/")[-1].split(".zip")[0]
            content = base_getters.get_content_from_url(url_prefix + url)
            z = zipfile.ZipFile(content)
            gml_files = [f for f in z.namelist() if f.endswith(".gml")]
            if gml_files:
                with z.open(gml_files[0]) as f:
                    s3.upload_fileobj(
                        Fileobj=f,
                        Bucket=bucket,
                        Key=f"source_data/inspire_ew/{area}.gml",
                    )
            time.sleep(1)


if __name__ == "__main__":
    main()
